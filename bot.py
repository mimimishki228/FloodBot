...
MIN_PLAYERS = 2
MAX_LIFE = 5
REGISTRATION_TIMEOUT = 300  # 5 минут в секундах
...
async def registration_timer(chat_id: int, message_id: int):
    """Таймер обратного отсчета для регистрации"""
    await asyncio.sleep(REGISTRATION_TIMEOUT)
    
    # Проверяем, активна ли еще регистрация
...
async def end_registration(chat_id: int, reason: str = "Регистрация завершена"):
    """Завершает регистрацию и начинает игру при достаточном количестве игроков"""
    if not game.is_registration_active:
        return
...
async def update_registration_message(chat_id: int):
    """Обновляет сообщение с регистрацией"""
...
async def cmd_start(message: types.Message):
    ...
    # Если регистрация уже идет, обновляем существующее сообщение
    ...
    # Если игра идет, показываем статус
    if game.is_active:
    ...
    # Отправляем сообщение с регистрацией
    text = f"🎮 <b>Набор в игру</b>\n\n"
...
async def cmd_stop(message: types.Message):
    ...
    # Если регистрация активна, завершаем её
    ...
    # Если игра активна, завершаем игру
...
async def cmd_faststart(message: types.Message):
    """Команда /faststart - быстрый старт игры (только админы)"""
    if message.chat.type not in ["group", "supergroup"]:
        await message.answer("Эта команда работает только в группах!")
        return

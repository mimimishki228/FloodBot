...
# Очки для победы в зависимости от количества игроков
WIN_SCORES = {
    2: 12,
    3: 17,
    4: 22
...
class GameState:
    def __init__(self):
        ...
        self.win_score: int = 22  # Будет установлено при старте игры
...
def get_player_status(user_id: int) -> str:
    ...
    return f"❤️{lives} | ⭐{score}/{game.win_score}"

def get_win_score() -> int:
    """Возвращает количество очков для победы в зависимости от количества игроков"""
    alive = len(get_alive_players())
    # Используем количество живых игроков для определения порога
    if alive <= 2:
        return WIN_SCORES[2]
    elif alive == 3:
        return WIN_SCORES[3]
...
🎲 <b>Как играть:</b>
1. Каждому игроку выпадают 5 случайных эмодзи
...
4. Игрок отправляет свой выбор в группу
...
async def start_new_game(chat_id: int):
    ...
    game.player_scores = {uid: 0 for uid in game.players}
    game.player_choices = {}
    ...
    # Устанавливаем количество очков для победы
...
async def start_player_turn(chat_id: int):
    ...
    # Обновляем очки (каждый раунд -2)
    for uid in game.players:
        if is_player_alive(uid):
            game.player_scores[uid] = game.player_scores.get(uid, 0) - 2
...
async def end_game(chat_id: int):
    ...
    if not alive_players:
        ...
        for uid in alive_players:
            score = game.player_scores.get(uid, 0)
            if score > max_score:
                max_score = score
                winner_id = uid
    ...
    text += f"Очки: {game.player_scores.get(winner_id, 0)}\n"
...
async def submit_choices(callback_query: types.CallbackQuery):
    ...
    for emoji, count in emoji_counts.items():
        if count >= 4:
            game.player_scores[user_id] = game.player_scores.get(user_id, 0) + 4
...
async def cancel_turn(callback_query: types.CallbackQuery):
    ...
    if user_id in game.player_choices:
        del game.player_choices[user_id]
...
async def judge_decision(callback_query: types.CallbackQuery):
    ...
    if decision == "true":
        if is_truth:
            ...
            game.player_scores[judge_id] = game.player_scores.get(judge_id, 0) - 2
            await handle_shot(current_player, chat_id, "игрок соврал", judge_id)
    else:  # false
        if is_truth:
            game.player_scores[judge_id] = game.player_scores.get(judge_id, 0) + 4
            await handle_shot(judge_id, chat_id, "судья ошибся", judge_id)
        else:
            game.player_scores[judge_id] = game.player_scores.get(judge_id, 0) + 4
            game.player_scores[current_player] = game.player_scores.get(current_player, 0) - 2
            await handle_shot(current_player, chat_id, "игрок соврал", judge_id)
...
async def handle_shot(player_id: int, chat_id: int, reason: str, judge_id: int = None):
    ...
    if shot_chance == 1:
        ...
        if lives <= 0:
            ...
            if judge_id:
                game.player_scores[judge_id] = game.player_scores.get(judge_id, 0) + 5
            ...
            game.player_scores[player_id] = game.player_scores.get(player_id, 0) + 3
